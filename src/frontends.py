...
def prepare_lfcc_double_delta(input):
    ...
    # 保证 window 参数和输入在同一设备
    if hasattr(LFCC_FN, 'Spectrogram') and hasattr(LFCC_FN.Spectrogram, 'window') and LFCC_FN.Spectrogram.window is not None:
        LFCC_FN.Spectrogram.window = LFCC_FN.Spectrogram.window.to(input.device)
    x = LFCC_FN(input)
    delta = delta_fn(x)
