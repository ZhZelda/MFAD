...
class GDTrainer(Trainer):
    def train(
        ...
        for epoch in range(self.epochs):
            ...
            num_total = 0.0
            model.eval()
            # 新增：收集所有 batch 的预测分数和标签用于 EER 计算
            all_y_pred = []
            all_y = []
            for batch_x, _, batch_y in test_loader:
                ...
                # 收集分数和标签
                all_y_pred.append(batch_pred.cpu())
            ...
            y_for_eer = 1 - all_y  # 与 evaluate_models.py 保持一致
            _, eer_val, _, _ = calculate_eer(y=y_for_eer, y_score=all_y_pred)
            LOGGER.info(
            ...
            torch.save(
                model.state_dict(),
                f"epoch_ckpt_epoch{epoch+1}.pth"
            )
