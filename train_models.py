...
def get_datasets(
    ...
    # datasets_paths: [asv_path, wavefake_path, celeb_path, asv19_path]
    asvspoof2021_df_path = datasets_paths[0]
    wavefake_path = datasets_paths[1] if len(datasets_paths) > 1 else None
    celeb_path = datasets_paths[2] if len(datasets_paths) > 2 else None
    asvspoof2019_la_path = datasets_paths[3] if len(datasets_paths) > 3 else None

    data_train = DetectionDataset(
        asvspoof_path=asvspoof2021_df_path,
        wavefake_path=wavefake_path,
        fakeavceleb_path=celeb_path,
        asvspoof2019_path=asvspoof2019_la_path,
        subset="train",
