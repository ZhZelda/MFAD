...
def get_df_dataset(dataset_path: Union[Path, str], amount_to_use: Optional[int]) -> DeepFakeASVSpoofDataset:
    data_val = DeepFakeASVSpoofDataset(
        path=dataset_path,
        subset="test",
    )
    return data_val
...
def parse_args():
    ...
    DF_DATASET_PATH = "D:\桌面\whisper_conformer\datasets\ASVspoof2021\DF"

    parser.add_argument(
        "--df_path",
        type=str,
        default=DF_DATASET_PATH,
        help="Path to DeepFakeASVSpoof dataset directory",
    )

    default_model_config = "D:\桌面\whisper_conformer\configs\model__whisper_multifront_conformer__1748358820.2918918.yaml"
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--amount",
        "-a",
        help=f"Amount of files to load from directory (default: {default_amount} - use all).",
        type=int,
